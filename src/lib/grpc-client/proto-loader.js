...
function deserializeStruct(message) {
  Object.keys(message).map((key) => {
    if (typeof message[key] === 'object') {
      ...
      message[key] = deserializeStruct(message[key]);
    }
  });
  return message;
}
function createDeserializer(cls, options) {
  return function deserialize(argBuf) {
    ...
    if (options.struct) {
      // Add struct options
      var message = cls.decode(argBuf);
      deserializeStruct(message);
      return cls.toObject(message, options);
    } else {
